...
R 节点身份卡只携带本次明确引用的来源键，其他来源保留在 Host 召回数据并向模型标注省略数量。至多八张身份卡的输入限额为 12 KiB；超出仍进入可见阻断。旧版 R 容量阻断仅在不存在命令、请求、屏障且已完成节点只有 S 时按 `r-bounded-cards-v2` 重试一次；有副作用或等待中的消息不得自动重试。
I 节点的事实与可用流程目录输入限额为 10 KiB；旧版 I 容量阻断仅在无命令、请求、屏障且既有节点都成功、其中包含 R 时按 `i-bounded-facts-v1` 重试一次。
