...
工程发现流程 v4 将完整路径清单压成目录与文件名组。切换实例时，已停在 `index-files` 容量等待的 v3 运行仅在旧节点已排空、此前节点成功、后续节点尚未启动且效果账已排空时，事务性迁移定义摘要与该节点输入；旧定义记录仍保留审计。启动后逐项回读原 Task 的 `index-files` 是否成功、`select-files` 是否启动；只见迁移回执或 Task 显示运行中不算完成。若迁移门禁拒绝，保留原运行和存储，先排查原因，不手工改 SQLite 或重复创建 Task。
