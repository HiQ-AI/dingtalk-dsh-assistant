...
R 节点身份卡只携带本次明确引用的来源键，其他来源保留在 Host 召回数据并向模型标注省略数量；前两张卡保留较完整目标、最近任务变化与结果，可通过 `task-history:<taskId>` 在同群、本人权限下按需取历史。至多八张身份卡的输入限额为 14 KiB；超出仍进入可见阻断。旧版 R 容量阻断仅在不存在命令、请求、屏障且已完成节点只有 S 时按 `r-bounded-cards-v2` 重试一次；有副作用或等待中的消息不得自动重试。
I 节点接收完整群职责、事实与可用流程目录，输入限额为 16 KiB；旧版 I 容量阻断仅在无命令、请求、屏障且既有节点都成功、其中包含 R 时按 `i-bounded-facts-v1` 重试一次。新消息先持久接收，再按控制账顺序一次处理一条；启动恢复沿用相同顺序，真正开始处理时才启动该消息的节点时间窗。渠道回读的自身发件按群和消息 ID 排除，不作为新业务消息；历史投影也排除这些回声。旧回声若没有业务命令，恢复时封存其等待请求，保留原记录。收发信箱合并新工作流账，只有通知独立回读后才显示已发送，撤回凭真实回执单独记录。
