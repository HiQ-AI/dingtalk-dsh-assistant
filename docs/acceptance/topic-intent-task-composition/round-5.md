...
| P04 | 真实平台客户端的只读连接、对象身份与客户端代码 | 待验收：Woodpecker token 进程内只读 /api/user 返回 200，repo 1 pipelines 第一页返回 50 条且字段包含 commit/branch/ref/status，Cron 返回 5 项并有 dataset-uat2-poll；UAT/生产 Deployment 与 Bytebase 目标已只读核对。当前 dataset 生产镜像 Registry digest 与两个 Ready Pod imageID 均为 sha256:9ee34b8bba770c8cd0867245957a4356afb2c85274037d4da40d510ce25f33b5；尚缺与源码 SHA 的同一链路证明及客户端完整能力 |
