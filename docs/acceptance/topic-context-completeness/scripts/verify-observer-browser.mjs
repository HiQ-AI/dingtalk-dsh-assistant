...
await page.route('**/*', async route => {
  ...
  let data = []
  if (target.pathname.endsWith('/document')) return route.fulfill({ status: 200, contentType: 'text/markdown', headers: { 'content-disposition': 'attachment; filename=proposal.md' }, body: '# 修改方案\n隔离方案文档' })
  if (target.pathname.includes('/nodes/') && target.pathname.endsWith('/output')) {
    ...
    return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ text: text.slice(cursor, end), nextCursor: end < text.length ? end : null, totalLength: text.length, overview: first ? '已读取 200 个文件' : '', documentName: first ? undefined : '修改方案.md' }) })
  }
...
try {
  ...
  await detail.getByRole('link', { name: '下载 修改方案.md' }).click()
  const download = await downloadEvent
  const downloadedPath = path.join(output, 'fixture-proposal.md')
  await download.saveAs(downloadedPath)
  assert.equal(await readFile(downloadedPath, 'utf8'), '# 修改方案\n隔离方案文档')
  checks.push('node-document-download')
