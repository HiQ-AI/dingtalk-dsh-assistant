...
const output = path.join(root, 'docs/acceptance/task-card-display/round-1')
...
try {
  ...
  for (const label of ['通知待生成', '通知受阻', '通知待送达', '通知已送达']) assert.ok(await page.getByRole('status').filter({ hasText: label }).count())
  const archive = page.locator('[data-task-card-action="archive-detail"]')
  assert.equal(await archive.count(), 1)
  assert.match(await archive.locator('summary').innerText(), /1 个工作目录 · 8 份文档/u)
  assert.equal(await archive.locator('code').isVisible(), false)
  await archive.locator('summary').click()
  assert.equal(await archive.locator('code').count(), 1)
  assert.equal(await archive.getByText('checklist-plan.csv', { exact: true }).count(), 1)
  const waiting = page.locator('[data-task-card-action="waiting-detail"]')
  assert.equal(await waiting.count(), 1)
  assert.ok(await page.getByText('等待 · 协调请求重试耗尽，等待系统恢复').count())
  await waiting.locator('summary').click()
  assert.match(await waiting.locator('code').innerText(), /topic_request_retry_exhausted/u)
