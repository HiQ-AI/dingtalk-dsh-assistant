...
> 状态：ACTIVE（本地部署完成，保留真实语义与外发验收）
> Goal ID：topic-driven-processing
> 最近维护：2026-09-17T22:25:00+08:00
> 权威目标：goal.md
...
| SG8 | 迁移、界面、文档与交付 | 存储升级可校验回退，API/Observer/文档同步，全量测试与 PR 交付 | 完成 | round-6.md；checklist-audit.md；PR #99 MERGED |
| SG9 | 实际 profile 迁移与本地部署 | 稳定备份、v7→v8 迁移、正式包安装、运行态与任务表完整回读 | 完成 | round-7.md；PR #100 MERGED |

## 当前检查点

- 当前子目标：实际 profile v7→v8 迁移与本地部署已完成。
- 下一步边界：按 `checklist-audit.md` 补齐生产模型冻结 gold 与经明确授权的真实 DWS 独立投递/群回读。
- 未闭环项：真实模型语义评测、真实 DWS 投递，以及附件页码/文档内部位置等更细来源能力。
...
- 2026-09-17：PR #99 已合并；真实 v7 预检发现 `attachmentId` 兼容缺口并由 PR #100 修复合并，全量测试 429/429。
- 2026-09-17：停机前确认 52 个 Task 全部完成；完成仓库外备份、v7→v8 迁移、正式包安装与启动。Runtime health、59 个 Topic、12 条任务表完整回读均通过；未发送真实群测试消息。
