...
本轮代码范围内的主链路已实现：一条消息可原子拆为多个 unit，分别归入 Topic、产生 Task、消费和反馈；一个事项完成后不等待同消息其他事项。正式回归 4/4、全量测试 400/400。

清单没有全部闭环。真实模型语义指标、实际安装目录增量比对、真实 profile 迁移、真实 DWS 投递读回、附件页码/段落坐标和历史合并 Task 的自动拆分计划仍需发布或后续能力验证，因此不得据此声称自然语言拆分“零遗漏”。
...
| SHARED-002、005～009、F2-1～F2-15 | PASS | UTF-16 半开区间、稳定 unitId/revision、显式 replacesUnitIds、固定 Topic revision、v7→v8 迁移及回执保留测试 | 真实 profile 副本尚未迁移 |
...
| F8-1 | NOT VERIFIED | 本轮未覆盖本地安装目录 | 发布前必须核对已安装增量，不能用源码或 main 直接覆盖 |
| F8-2～F8-10 | PASS（本地） | 诊断脚本已改为正式目标断言；#1066 拆为 3 个目标，四项回归保持一个目标；迁移、并发、重启、部分失败由测试覆盖 | 实际 profile 与真实群不在本轮本地测试范围 |
| F8-12 | PASS（样本冻结） | `fixtures/message-multi-topic-gold.json` 覆盖应拆、应合、背景、混合授权、纠正和对抗改写 | 尚未用生产配置模型跑样本 |
| F8-13～F8-15 | NOT VERIFIED | 已区分 Host 结构测试和语义评测 | 未取得过度拆分率、漏拆率、错误续接率、授权扩大率，也未验证已部署增量集成差异和历史 Task 成果分配 |
| F8-16 | PASS | 文档和交付结论均保留语义边界 | — |

## 发布前必须补证

1. 读取实际安装目录与运行 profile 的版本、文件差异及 domain 版本，再决定迁移和部署步骤。
2. 使用生产配置模型运行冻结 gold，分别统计过度拆分、漏拆、错误续接和授权扩大。
3. 在隔离副本执行 v7→v8 `--check`、迁移、重开和指纹比对，再切换实际 profile。
4. 在获准的真实群验证三个事项独立确认、独立完成通知、单条失败隔离及群消息读回。
5. 若本期要求完整满足 F6-6、F7-1 或超大路由分段提交，需继续实现后再发布。
