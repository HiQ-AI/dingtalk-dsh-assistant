...
| SG11 | 默认处理模型热更新 | 活动Task期间可持久保存，下一次模型请求生效，工作目录门禁保持 | 已完成 | round-22.md；633/633；8个真实请求 |

## 当前检查点

- 当前子目标：SG11
- 唯一下一步：SG11已交付，PR #119保持OPEN等待审阅；继续由现有业务Task运行。不自动合并PR。
- 未闭环项：PR #119尚未合并；真实渠道D03、真实端到端时延及既有业务Task结果仍独立验收。真实渠道D03、真实端到端时延及三个业务目标本身仍由各Task推进。
...
- 2026-09-23 SG11基线：PR #118已合并ee06851，新分支feature/default-processing-model基于origin/main。当前API/profile默认模型gpt-5.6-sol；7条非终态Task存在时PUT gpt-6-sol稳定返回400。根因是模型选择与工作目录共用agent_config_has_active_tasks门禁，保存未发生。

- SG11实现：解除模型选择的活动Task门禁，工作目录门禁保持；Runtime每次从DSH原生服务读取选择，提示组装快照保护在途请求。针对性3/3、全量633/633通过。

- SG11部署：f9e67af精确包已安装，34文件hash一致；API和持久设置均为gpt-6-sol，8个不同Session部署后真实request/context均使用该模型。PID113788，Web200、health ok、告警0、入站和桥接正常。
