...
- **推理深度**：可选“模型默认、轻度、中度、高度、极高”。模型和推理深度通过 DSH 原生设置持久化，活动 Task 存在时也可保存；已经完成提示组装的在途请求保持原选择，后续模型请求读取新值。
