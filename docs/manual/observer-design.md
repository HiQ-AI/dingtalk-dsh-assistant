...
顶部显示中文状态、标题、更新时间与真实步骤完成数；主体左侧为阶段和编号时间线，右侧为待处理事项、最新产出与目标。步骤编号始终保留，当前/等待/失败步骤使用对应状态色强调，完成与未开始步骤降低强调；窄屏切为单栏。最新产出不等于已送达；失败和取消不得标成已完成。隐藏工作流编号与原始数据；历史执行和会话在展开后请求，首屏不加载。目标/产出自然换行，空步骤与无产出明确提示。
