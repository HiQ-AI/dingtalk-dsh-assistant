...
真实 stale 是乐观并发协议的预期分支，不是工具参数或业务执行异常。现场 `group_reply_submit` 也复现了这一分支：通知生成后新群消息进入，旧通知先被安全拒绝，普通消息处理完成后同 turn 重提成功。当前实现通过抛异常返回，导致 DSH 把一次安全拒绝展示成 `Assistant Message / Failed`；即使下一 step 自动恢复，失败卡片仍会留在会话里。
