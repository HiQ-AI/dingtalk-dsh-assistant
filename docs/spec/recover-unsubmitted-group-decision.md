...
2026-09-04 09:29:03 至 09:29:17，同群连续三条消息 `#589`、`#590`、`#591` 均在约五分钟后落为 `decision-failed`，错误统一为 `group_decision_not_submitted`。其中 `#589` 是本插件此前写入 pending Outbox 的短引用回复，钉钉回读正文额外带有前导 `@当前用户`；`#590` 是明确的新任务请求；`#591` 引用 `#589` 表示该旧询问无需处理。
