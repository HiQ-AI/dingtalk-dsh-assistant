...
- Agent 发件箱：在入站群聊消息下方按持久化顺序倒序展示 outbox，每页固定 10 条；展示消息正文、来源消息 ID、回复目标、真实投递消息 ID 和 Outbox ID。历史 outbox 默认展示“已回读确认”，缺少真实 messageId 时显示“历史未记录”；新建 outbox 通过 `readbackRequired` 将主会话回复与回读确认绑定，在真实 messageId 持久化前展示“待回读确认”，Runtime 持续使用同一 Outbox ID 对账，回读命中后转为“已回读确认”。outbox 没有发送时间字段，因此页面不推测发送时间。
