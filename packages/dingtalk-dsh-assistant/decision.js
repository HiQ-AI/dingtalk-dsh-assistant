...
const taskContext = z.strictObject({ kind: z.literal('task-context'), taskId: z.string().min(1), ...taskVersion, context: z.string().min(1), objective: z.string().min(1).optional(), progressImpact: z.enum(['preserve', 'replan']).optional(), ...runPlan, ...taskSources })
const taskReopen = z.strictObject({ kind: z.literal('task-reopen'), taskId: z.string().min(1), ...taskVersion, context: z.string().min(1), objective: z.string().min(1).optional(), ...runPlan, ...taskSources })
...
const taskActionJsonSchema = {
  oneOf: [
    ...
    { type: 'object', additionalProperties: false, properties: { kind: { type: 'string', const: 'task-context' }, taskId: stringJsonSchema, ...taskVersionJsonProperties, context: stringJsonSchema, objective: stringJsonSchema, progressImpact: { type: 'string', enum: ['preserve', 'replan'] }, ...runPlanJsonProperties, topicRefs: topicRefsJsonProperty }, required: ['kind', 'taskId', 'inputVersion', 'runSequence', 'context', 'topicRefs'] },
    { type: 'object', additionalProperties: false, properties: { kind: { type: 'string', const: 'task-reopen' }, taskId: stringJsonSchema, ...taskVersionJsonProperties, context: stringJsonSchema, objective: stringJsonSchema, ...runPlanJsonProperties, topicRefs: topicRefsJsonProperty }, required: ['kind', 'taskId', 'inputVersion', 'runSequence', 'context', 'topicRefs'] },
...
export function blockTaskDecisionForUnavailableMedia(decision, mediaUnavailable) {
  ...
  return { basisMessageIds: decision.basisMessageIds, actions: [], reply: `我没能获取到以下任务信息：${unavailable.join('；')}。请重新发送可访问的内容，信息补齐后我再开始处理。`, ...(decision.replyReview ? { replyReview: decision.replyReview } : {}) }
}
