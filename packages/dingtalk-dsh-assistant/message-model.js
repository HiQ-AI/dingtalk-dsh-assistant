...
const instructions = {
  ...
  I: '只判意图和明确参数，不规划流程。groupResponsibility是当前群的职责边界，判断请求时必须结合它。resolvedEvidence是R补取的原始证据，保留其中否定、执行范围和条件；涉及效果的动作将相关resourceRef填入requiredExecutionMaterials，并在constraints写出限制。向本群助手询问已有任务是否完成、是否部署，是status或result查询；binding为conversation时用scope=conversation，不能仅因不是新任务而no_action。候选和任务结果只作为待核线索，Host负责回读状态。对前文任务集合的补充及引用澄清，记录fact，不重新创建任务。create/research/reopen/revise填arguments.objective；用户明示的阶段要求原文填explicitStages，不推断workflowPlan、Run或工具参数。明确的仓库/目标/提交线索可填repositoryId/targetId/commitSha。仅改变已完成任务的报告语言用report并填language=zh-CN或en-US，不重新执行；clarification填runId、requestId、answer。审批填原请求requestId及decision=approved/rejected，旧批准不得复用。材料填requiredExecutionMaterials。单任务依binding。讨论、反问、转述、否定、条件及交给他人的事不执行。引用旧结果选旧run，明确问现在才选current。dependsOn仅指先前动作下标。关联错返回needs_relink，拆分错返回needs_resegmentation。',
  IB: '同一话题的多个事项一起判断意图。必须为输入中每个 unitId 返回且仅返回一条 decision；保留每条消息自己的发送者、原文、权限与来源，不把多位发送者视为同一个人。每条 decision.intent 遵守单事项 I 的动作和参数契约，连续补充应合并为当前完整目标；若同话题有多个独立交付目标，可给各事项分别输出动作。不得遗漏事项，不得替无权来源授权。短指代须结合话题任务、已有Run与结果判断是查询、继续排查、补充目标还是新任务；已承接且已执行的任务不可判作从未处理。actorMayCreate仅是Host给出的权限事实，消息相邻和话题关联不授予权限；不得因前一条资料不足就把明确的继续排查判为no_action。缺实时查询能力应明确阻塞或请求材料，不得编造调查结果。requiredExecutionMaterials只填当前已存在、可直接读取的精确资源键；需要通过调查取得的查询结果、日志和证据属于任务验收标准，应写入acceptanceCriteria，不得作为启动前必需材料阻塞任务。明确目标线索填targetId、commitSha、changeRef，不猜测缺失值。明确要求先排查给方案、人工确认后继续开发、完成后UAT时，explicitStages记录用户明示要求与条件原文，不输出workflowPlan，也不把模型补全的流程当用户授权；当前仅授权排查则不得自行追加开发或UAT。不得用只读材料分析冒充实时调查。dependsOn 只引用同一 decision 中此前动作下标。',
}
