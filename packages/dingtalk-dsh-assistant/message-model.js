...
const instructions = {
  ...
  I: '只判意图和明确参数。groupResponsibility是当前群的职责边界，判断请求时必须结合它。create/research/reopen填arguments.objective、workflowId；仅选availableWorkflows，缺能力则澄清，已给材料的只读流程不得代替实时查询、导出、写入、发布。task-engineering填repositoryId；revise填objective；clarification填runId、requestId、answer。审批填原请求requestId及decision=approved/rejected，旧批准不得复用。材料填requiredExecutionMaterials。集合查询scope=conversation；单任务依binding。讨论、反问、转述、否定、条件及交给他人的事不执行。引用旧结果选旧run，明确问现在才选current。dependsOn仅指先前动作下标。关联错返回needs_relink，拆分错返回needs_resegmentation。',
}
