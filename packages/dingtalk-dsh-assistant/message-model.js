...
const instructions = {
  ...
  R: '只关联当前事项与候选身份卡。引用不等于授权。已有目标必须选给定candidateId；不能凭标题猜同一对象。对“这不是让你去查吗”等紧邻短指代，先核对当前source原文、最近同一发送人的消息及其话题卡；S的goalText不得替代source原文，也不得凭更早的多项任务推翻明确的紧邻指代。omissions表示标题或目标被裁剪，不代表剩余部分无关；若候选有historyRef且历史会影响判断，先返回needs_context并以historyRef作为resourceRef，不能直接向人重复追问。omittedCandidateCount大于零时没看到候选不能证明没有已有任务；对指代不明的事项不能仅因可见候选不匹配而新建。对“我的审核问题”“这批任务”等集合状态查询可选conversation；结合最近消息消解“这两个”“它”，不要把自己发出的澄清当用户的新请求。仍无法确定且不能按集合回答时再澄清。',
  ...
  IB: '同一话题的多个事项一起判断意图。必须为输入中每个 unitId 返回且仅返回一条 decision；保留每条消息自己的发送者、原文、权限与来源，不把多位发送者视为同一个人。每条 decision.intent 遵守单事项 I 的动作和参数契约，连续补充应合并为当前完整目标；若同话题有多个独立交付目标，可给各事项分别输出动作。不得遗漏事项，不得替无权来源授权。用户紧接问题说“这不是让你去查吗”且binding已核实最近同人来源、actorMayCreate=true时，是对该话题的明确排查交办，不得因前一条仅回答资料不足而判为no_action；缺实时查询能力应明确阻塞或请求材料，不得编造调查结果。requiredExecutionMaterials只填当前已存在、可直接读取的精确资源键；需要通过调查取得的查询结果、日志和证据属于任务验收标准，应写入action.arguments.acceptanceCriteria，不得作为启动前必需材料阻塞任务。明确要求先排查给方案、人工确认后继续开发、完成后UAT时，workflowPlan用有序阶段表示，需确认处gate=confirmation；当前仅授权排查则不得自行追加开发或UAT。未固化事项可选择可用的task-general。dependsOn 只引用同一 decision 中此前动作下标。',
}
