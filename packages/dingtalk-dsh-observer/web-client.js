window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function TopicBrowser({ groups, tasks, target, updatedAt, onOpenTask }) {
      ...
      const selectTopic = (topic) => { setSelection({ groupId: topic.groupId, topicId: topic.topicId, revision: topic.revision }); setMessageOffset(0); setMessagesExpanded(false) }
      ...
      return React.createElement('section', { 'aria-label': '话题与上下文', style: { display: 'grid', gap: 16 } },
        React.createElement('div', { style: { ...toolbar, flexWrap: 'wrap' } }, React.createElement(SelectMenu, { label: '筛选话题群聊', value: groupId, options: [{ id: '', label: '全部群聊' }, ...groups.map((item) => ({ id: item.groupId, label: item.name || item.groupId }))], onChange: (value) => { setGroupId(value); setOffset(0); setListing(undefined); setSelection(undefined); setMessagesExpanded(false) } }), React.createElement('span', { style: { fontSize: 12, color: colors.muted } }, group?.topicProgress ? `待归类 ${group.topicProgress.unroutedMessages} 条 · 待处理 ${group.topicProgress.pending} 个话题` : '接收、话题处理与任务完成分别计量')),
        React.createElement('div', { style: { display: 'flex', flexWrap: 'wrap', alignItems: 'flex-start', gap: 16 } },
          ...
          React.createElement('section', { 'aria-label': '话题详情', 'aria-busy': detailLoading, style: { ...tableFrame, flex: '2 1 560px', width: '100%', minWidth: 0, minHeight: 200 } },
            ...
            detailError ? failure(`上下文加载失败：${detailError}`) : detailLoading ? React.createElement('div', { role: 'status' }, emptyState('正在读取固定版本上下文…')) : !context ? emptyState('选择一个话题查看消息和关联任务') : React.createElement('div', { style: { minWidth: 0, padding: '0 18px 18px', display: 'grid', gap: 0, overflowWrap: 'anywhere' } },
              ...
              React.createElement('details', { key: `${selection.topicId}:${selection.revision}`, open: messagesExpanded, onToggle: (event) => setMessagesExpanded(event.currentTarget.open), style: { marginTop: 4, borderTop: `1px solid ${colors.border}` } },
                React.createElement('summary', { style: { padding: '14px 0', cursor: 'pointer', fontSize: 13, fontWeight: 600 } }, `引用消息 · ${context.total} 条`),
                ...
                pager(messageOffset, context.total, setMessageOffset, detailLoading))
            ))))
    }
