  ...
  factory: (require) => {
    ...
    function ObserverContent({ openSession }) {
      ...
      const renderTaskCard = (task) => {
        ...
        const checkpoints = task.stageTasks?.length ? task.stageTasks : (planCheckpoint?.remainingItems?.length ? planCheckpoint.remainingItems : [task.objective])
        const displayObjective = String(task.objective || '').startsWith('[TASK_SOURCE_EVIDENCE]') ? (task.title || task.objective) : task.objective
        const completedCheckpointNames = new Set(task.state === 'completed' ? checkpoints : checkpointEvents.filter((item) => item.kind === 'stage-completed' && ['acknowledge', 'guidance'].includes(item.coordinatorDecision)).map((item) => item.stageTask).filter((item) => checkpoints.includes(item)))
