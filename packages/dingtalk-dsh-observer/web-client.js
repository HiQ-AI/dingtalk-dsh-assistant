      ...
      const renderTaskCard = (task) => {
        ...
        return React.createElement('div', { key: task.taskId, onMouseEnter: () => setHoveredTaskId(task.taskId), onMouseLeave: () => setHoveredTaskId(''), style: { width: '100%', minWidth: 0, boxSizing: 'border-box', border: `1px solid ${colors.border}`, borderRadius: 12, background: colors.cardSurface, boxShadow: hovered ? '0 8px 20px rgba(15,23,42,.08), 0 2px 6px rgba(15,23,42,.05)' : 'var(--dsw-shadow-card, 0 1px 2px rgba(0,0,0,.08))', padding: '11px 13px', color: 'inherit', display: 'grid', gap: 6, opacity: queued ? 0.72 : 1, transition: 'box-shadow 180ms ease' } },
          ...
          React.createElement('div', null,
            task.workflowProgress?.outcomeLabel ? React.createElement('div', { role: 'status', style: { fontSize: 11, color: task.workflowProgress.outcome === 'succeeded' ? statusTone.done : colors.muted } }, task.workflowProgress.outcomeLabel) : null,
            (task.notificationIntents || []).filter(intent => intent.inputVersion === task.inputVersion && intent.runSequence === task.runSequence && intent.status !== 'superseded').map(intent => React.createElement('div', { key: intent.intentId, role: 'status', style: { fontSize: 10.5, color: intent.status === 'blocked' ? colors.warning : colors.muted } }, ({ pending: '完成通知待生成', enqueued: '完成通知待送达', delivered: '完成通知已送达', blocked: '完成通知受阻' })[intent.status])),
