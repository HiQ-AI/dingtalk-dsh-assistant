  ...
  factory: (require) => {
    ...
    function TopicBrowser({ groups, tasks, target, updatedAt, onOpenTask }) {
      ...
      useEffect(() => {
        ...
        get(`/state/topics?${params}`).then((value) => { if (active) { setListing(value); if (value.total > 0 && offset >= value.total) setOffset(Math.floor((value.total - 1) / pageSize) * pageSize) } }, (error) => { if (active) setListError(error.message) }).finally(() => { if (active) setLoading(false) })
        return () => { active = false }
      ...
      useEffect(() => {
        ...
        const params = new URLSearchParams({ groupId: selection.groupId, revision: String(selection.revision), offset: String(messageOffset), limit: String(pageSize) })
      ...
      const topicStatus = (item) => item.processing ? item.processing.status === 'failed' ? { label: '处理失败', state: 'error' } : { label: '处理中', state: 'ongoing' } : item.status === 'closed' ? { label: '已结束', state: 'done' } : item.status === 'waiting' ? { label: '等待中', state: 'warning' } : { label: '进行中', state: 'ongoing' }
      const topicProgress = (item) => Math.round((Math.min(item.processedRevision || 0, item.revision || 0) / Math.max(1, item.revision || 0)) * 100)
      ...
      const pager = (pageOffset, total, onChange, busy) => React.createElement('div', { style: { ...tableFooter, flexWrap: 'wrap' } }, React.createElement('span', { style: { marginRight: 'auto', color: colors.muted, fontSize: 12 } }, `${total} 条 · 第 ${Math.floor(pageOffset / pageSize) + 1} 页`), React.createElement(Button, { variant: 'outline', size: 'sm', type: 'button', disabled: busy || pageOffset === 0, onClick: () => onChange(Math.max(0, pageOffset - pageSize)) }, '上一页'), React.createElement(Button, { variant: 'outline', size: 'sm', type: 'button', disabled: busy || pageOffset + pageSize >= total, onClick: () => onChange(pageOffset + pageSize) }, '下一页'))
      return React.createElement('section', { 'aria-label': '话题与上下文', style: { display: 'grid', gap: 16 } },
        ...
        React.createElement('div', { style: { display: 'flex', flexWrap: 'wrap', alignItems: 'flex-start', gap: 16 } },
          React.createElement('section', { 'aria-label': '话题列表', 'aria-busy': loading, style: { ...tableFrame, flex: '1 1 340px', width: '100%', minWidth: 0, maxWidth: 420, minHeight: 200 } },
            ...
            listError ? failure(`话题加载失败：${listError}`) : !listing ? emptyState('正在读取话题…') : listing.topics.length === 0 ? emptyState('暂无话题，消息归类后会显示在这里') : React.createElement('ul', { style: { listStyle: 'none', margin: 0, padding: 0 } }, ...listing.topics.map((item) => { const selected = selection?.topicId === item.topicId && selection?.groupId === item.groupId; const status = topicStatus(item); const progress = topicProgress(item); return React.createElement('li', { key: `${item.groupId}:${item.topicId}`, style: { borderBottom: `1px solid ${colors.border}` } }, React.createElement('button', { type: 'button', 'aria-pressed': selected, 'aria-label': item.title || item.topicId, onClick: () => selectTopic(item), style: { position: 'relative', width: '100%', minWidth: 0, border: 0, borderLeft: `3px solid ${selected ? colors.accent : 'transparent'}`, padding: '14px 14px 13px 13px', background: selected ? `color-mix(in srgb, ${colors.accent} 8%, ${colors.cardSurface})` : colors.cardSurface, color: 'inherit', cursor: 'pointer', textAlign: 'left', fontFamily: 'inherit', outlineOffset: -2 } },
              React.createElement('div', { style: { display: 'flex', alignItems: 'flex-start', gap: 10 } }, React.createElement('strong', { title: item.title || item.topicId, style: { minWidth: 0, flex: '1 1 auto', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden', overflowWrap: 'anywhere', fontSize: 13, fontWeight: 600, lineHeight: 1.55 } }, item.title || item.topicId), React.createElement('span', { style: { flex: '0 0 auto', display: 'inline-flex', alignItems: 'center', gap: 5, color: statusTone[status.state] || colors.muted, fontSize: 11, lineHeight: '20px', whiteSpace: 'nowrap' } }, React.createElement(StateDot, { state: status.state, size: 7 }), status.label)),
              item.summary ? React.createElement('p', { title: item.summary, style: { margin: '7px 0 0', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden', overflowWrap: 'anywhere', whiteSpace: 'pre-wrap', color: colors.muted, fontSize: 11.5, lineHeight: 1.55 } }, item.summary) : null,
              React.createElement('div', { style: { display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) auto', alignItems: 'center', gap: 10, marginTop: 10 } }, React.createElement('div', { role: 'progressbar', 'aria-label': `话题版本处理进度 ${item.processedRevision || 0}/${item.revision || 0}`, 'aria-valuemin': 0, 'aria-valuemax': 100, 'aria-valuenow': progress, style: { minWidth: 32, height: 3, borderRadius: 999, overflow: 'hidden', background: `color-mix(in srgb, ${colors.muted} 20%, transparent)` } }, React.createElement('div', { style: { width: `${progress}%`, height: '100%', borderRadius: 'inherit', background: status.state === 'error' ? colors.danger : colors.accent } })), React.createElement('span', { style: { color: colors.muted, fontSize: 10.5, fontVariantNumeric: 'tabular-nums', whiteSpace: 'nowrap' } }, `版本 ${item.processedRevision || 0} / ${item.revision || 0}${item.processing ? ` · 动作 ${item.processing.appliedOperations} / ${item.processing.totalOperations}` : ''}`)))
            ) })),
            listing ? pager(offset, listing.total, setOffset, loading) : null),
          React.createElement('section', { 'aria-label': '话题详情', 'aria-busy': detailLoading, style: { ...tableFrame, flex: '2 1 560px', width: '100%', minWidth: 0, minHeight: 200 } },
            React.createElement('header', { style: { padding: '17px 18px 15px', borderBottom: `1px solid ${colors.border}`, background: colors.cardSurface } }, React.createElement('div', { style: { display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '6px 12px', marginBottom: selection ? 8 : 0 } }, topic ? React.createElement('span', { style: { display: 'inline-flex', alignItems: 'center', gap: 5, color: statusTone[topicStatus(topic).state] || colors.muted, fontSize: 11.5, whiteSpace: 'nowrap' } }, React.createElement(StateDot, { state: topicStatus(topic).state, size: 7 }), topicStatus(topic).label) : null, selection ? React.createElement('span', { style: { color: colors.muted, fontSize: 11.5, fontVariantNumeric: 'tabular-nums' } }, `固定版本 ${selection.revision}`) : null), React.createElement('h2', { title: topic?.title, style: { minWidth: 0, margin: 0, overflowWrap: 'anywhere', fontSize: 18, fontWeight: 650, lineHeight: 1.5, letterSpacing: '-.01em' } }, topic?.title || '话题详情')),
            detailError ? failure(`上下文加载失败：${detailError}`) : detailLoading ? React.createElement('div', { role: 'status' }, emptyState('正在读取固定版本上下文…')) : !context ? emptyState('选择一个话题查看消息和关联任务') : React.createElement('div', { style: { minWidth: 0, padding: '0 18px 18px', display: 'grid', gap: 0, overflowWrap: 'anywhere' } },
              topic?.summary ? React.createElement('section', { 'aria-label': '话题摘要', style: { marginTop: 16, padding: '13px 14px', borderLeft: `3px solid ${colors.accent}`, borderRadius: '0 9px 9px 0', background: `color-mix(in srgb, ${colors.accent} 6%, ${colors.surface2})` } }, React.createElement('div', { style: { color: colors.muted, fontSize: 10.5, fontWeight: 600 } }, `当前摘要 · 覆盖版本 ${topic.summaryRevision ?? topic.revision}`), React.createElement('p', { style: { margin: '6px 0 0', whiteSpace: 'pre-wrap', fontSize: 13, lineHeight: 1.65 } }, topic.summary)) : null,
              React.createElement('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 240px), 1fr))', gap: 12, padding: '16px 0', borderBottom: `1px solid ${colors.border}` } },
                ...
                React.createElement('section', { 'aria-label': '关联任务', style: { minWidth: 0 } }, React.createElement('strong', { style: { display: 'block', marginBottom: 8, fontSize: 12 } }, `关联任务 · ${relatedTasks.length}`), React.createElement('div', { style: { display: 'flex', flexWrap: 'wrap', gap: 7 } }, ...relatedTasks.map((task) => React.createElement(Button, { key: task.taskId, variant: 'outline', size: 'sm', type: 'button', disabled: !task.childSessionId || task.state === 'queued', onClick: () => onOpenTask(task), style: { maxWidth: '100%' } }, React.createElement('span', { style: { overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' } }, task.title || task.objective))), relatedTasks.length === 0 ? React.createElement('span', { style: { color: colors.muted, fontSize: 12 } }, '暂无关联任务') : null))),
              React.createElement('div', { style: { display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 12, padding: '16px 0 8px' } }, React.createElement('strong', { style: { fontSize: 13 } }, '原始输入'), React.createElement('span', { style: { color: colors.muted, fontSize: 11, fontVariantNumeric: 'tabular-nums', whiteSpace: 'nowrap' } }, `版本 ${context.revision} · ${context.total} 条消息`)),
              React.createElement('div', { style: { display: 'grid' } }, ...(context.messages?.length ? context.messages.map((message, index) => React.createElement('article', { key: `${message.messageId}:${index}`, style: { position: 'relative', minWidth: 0, padding: '10px 0 15px 22px', borderLeft: `1px solid ${colors.border}` } }, React.createElement('span', { 'aria-hidden': true, style: { position: 'absolute', top: 15, left: -4, width: 7, height: 7, borderRadius: '50%', background: index === 0 ? colors.accent : colors.cardSurface, border: `1px solid ${index === 0 ? colors.accent : colors.border}` } }), React.createElement('div', { style: { display: 'flex', flexWrap: 'wrap', alignItems: 'baseline', gap: '2px 8px', color: colors.muted, fontSize: 11 } }, React.createElement('strong', { style: { color: 'inherit', fontWeight: 600 } }, message.senderName || message.sourceKind || '未知发送人'), React.createElement('time', { dateTime: message.occurredAt || undefined, style: { fontVariantNumeric: 'tabular-nums' } }, fmt(message.occurredAt))), React.createElement('p', { style: { margin: '6px 0 0', whiteSpace: 'pre-wrap', overflowWrap: 'anywhere', fontSize: 13, lineHeight: 1.7 } }, message.text || '（空消息）'), message.quotedMessage?.content ? React.createElement('blockquote', { style: { color: colors.muted, margin: '9px 0 0', padding: '8px 10px', borderLeft: `2px solid ${colors.border}`, background: colors.surface2, whiteSpace: 'pre-wrap', overflowWrap: 'anywhere', fontSize: 12, lineHeight: 1.6 } }, message.quotedMessage.content) : null)) : [emptyState('此版本没有消息')]))
            ), context ? pager(messageOffset, context.total, setMessageOffset, detailLoading) : null)))
    }
    function ObserverContent({ openSession }) {
      ...
      const renderTaskCard = (task) => {
        ...
        return React.createElement('div', { key: task.taskId, onMouseEnter: () => setHoveredTaskId(task.taskId), onMouseLeave: () => setHoveredTaskId(''), style: { width: '100%', minWidth: 0, boxSizing: 'border-box', border: `1px solid ${colors.border}`, borderRadius: 12, background: colors.cardSurface, boxShadow: hovered ? '0 8px 20px rgba(15,23,42,.08), 0 2px 6px rgba(15,23,42,.05)' : 'var(--dsw-shadow-card, 0 1px 2px rgba(0,0,0,.08))', padding: '11px 13px', color: 'inherit', display: 'grid', gap: 6, opacity: queued ? 0.72 : 1, transition: 'box-shadow 180ms ease' } },
          ...
          task.topicRefs?.length ? React.createElement('div', { 'aria-label': '任务关联话题', style: { display: 'flex', flexWrap: 'wrap', gap: 6 } }, ...task.topicRefs.map((ref) => React.createElement(Button, { key: ref.topicId, variant: 'outline', size: 'sm', type: 'button', onClick: () => { setTopicTarget({ groupId: task.groupId, topicId: ref.topicId, revision: ref.revision }); setActivePage('topics') } }, `话题 ${short(ref.topicId)} · v${ref.revision}`))) : null)
      }
