  ...
  factory: (require) => {
    ...
    function ObserverContent({ openSession }) {
      ...
      const renderTaskCard = (task) => {
        ...
        const documentCount = worktrees.reduce((count, item) => count + new Set((item.documents || []).map(document => document.source)).size, 0)
        ...
        for (const intent of notifications) notificationCounts.set(intent.status, (notificationCounts.get(intent.status) || 0) + 1)
        const retryExhausted = task.waitingReason?.startsWith('topic_request_retry_exhausted:')
        const waitingLabel = retryExhausted ? '协调请求重试耗尽，等待系统恢复' : task.waitingReason
        ...
        return React.createElement('div', { key: task.taskId, onMouseEnter: () => setHoveredTaskId(task.taskId), onMouseLeave: () => setHoveredTaskId(''), style: { width: '100%', minWidth: 0, boxSizing: 'border-box', border: `1px solid ${colors.border}`, borderRadius: 12, background: colors.cardSurface, boxShadow: hovered ? '0 8px 20px rgba(15,23,42,.08), 0 2px 6px rgba(15,23,42,.05)' : 'var(--dsw-shadow-card, 0 1px 2px rgba(0,0,0,.08))', padding: '11px 13px', color: 'inherit', display: 'grid', gap: 6, opacity: queued ? 0.72 : 1, transition: 'box-shadow 180ms ease' } },
          ...
          React.createElement('div', null,
            task.workflowProgress?.outcomeLabel || notifications.length ? React.createElement('div', { role: 'status', style: { display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 5, fontSize: 11, lineHeight: 1.5 } },
          ...
          task.waitingReason ? React.createElement('div', { style: { fontSize: 11, lineHeight: 1.5, color: colors.warning, overflowWrap: 'anywhere' } }, `等待 · ${waitingLabel}`,
            retryExhausted ? React.createElement('details', { 'data-task-card-action': 'waiting-detail', style: { marginTop: 3, color: colors.muted, fontSize: 10.5 } }, React.createElement('summary', { style: { cursor: 'pointer' } }, '查看技术原因'), React.createElement('code', { style: { display: 'block', marginTop: 4, overflowWrap: 'anywhere' } }, task.waitingReason)) : null,
          ...
          worktrees.length || cleanup ? React.createElement('section', { 'aria-label': '本地工作目录归档', style: { minWidth: 0, padding: '7px 0', borderTop: `1px solid ${colors.border}`, fontSize: 11, lineHeight: 1.5 } },
            React.createElement('details', { 'data-task-card-action': 'archive-detail', style: { minWidth: 0 } },
              React.createElement('summary', { style: { cursor: 'pointer', color: cleanup?.status === 'failed' ? colors.danger : cleanupRunning ? colors.warning : colors.muted, fontWeight: 600, overflowWrap: 'anywhere' } }, `${cleanupLabel} · ${worktrees.length} 个工作目录 · ${documentCount} 份文档`),
              React.createElement('div', { style: { display: 'grid', gap: 9, minWidth: 0, marginTop: 8, paddingLeft: 12 } },
                cleanup?.error ? React.createElement('span', { role: 'alert', style: { color: colors.danger, overflowWrap: 'anywhere' } }, cleanup.error) : null,
                ...worktrees.map(worktree => React.createElement('div', { key: worktree.path, style: { display: 'grid', gap: 3, minWidth: 0 } },
                  React.createElement('span', { style: { color: colors.muted } }, `${worktree.status === 'cleaned' ? '已清理' : '已登记'}${worktree.createdByTask ? '' : ' · 借用'}${worktree.branch ? ` · ${worktree.branch}` : ''}`),
                  React.createElement('code', { title: worktree.path, style: { overflowWrap: 'anywhere' } }, worktree.path),
                  ...[...new Map((worktree.documents || []).map(document => [document.source, document])).values()].map(document => React.createElement('div', { key: document.source, title: document.source, style: { minWidth: 0, paddingLeft: 8, borderLeft: `1px solid ${colors.border}`, color: colors.muted, overflowWrap: 'anywhere' } },
                    React.createElement('span', null, document.source.split(/[\\/]/).at(-1)),
                    document.archivePath ? React.createElement('div', { title: document.archivePath, style: { fontSize: 10.5, overflowWrap: 'anywhere' } }, `归档到 ${document.archivePath}`) : null))))))) : null,
