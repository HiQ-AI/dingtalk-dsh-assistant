...
- 历史回复审阅候选增加 8 条/16 KB 硬预算，并在 Resident 恢复时清理依赖旧内存请求表的待处理协议信封；连续超出上下文窗口的旧 Resident 会保留日志并自动换新 Session，旧版退出产生的可证明无副作用失败自动迁移到顺序恢复，避免长历史群聊因单条 Steer 膨胀而反复压缩、重启后仍不处理。
