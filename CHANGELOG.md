...
- 协调请求增加 `superseded` 持久终态，完成与失效请求均可收口；相同恢复故障按稳定身份聚合计数，已失效的结果快照不再污染当前健康状态。
...
- 为常驻会话增加只读状态短答路径；在任务、职责和 Topic 快照保持一致时，直接复用当前模型生成简短答复，减少完整协调回合的等待。
- 持久化叶子报告的接收、处理和通知状态；重启后可恢复尚未通知的结果，失败报告按原提交身份显式重试。

### 变更

- 常驻与叶子统一使用带版本和来源的当前事实、阶段计划与审批快照；任务输入变化只使受影响阶段失效，历史结果保留为历史事实。
- 审阅与流程正文只从当前 Session 可见的结构化工具结果复用；压缩或内容变化后重新读取，避免重复注入和过期上下文继续生效。
- 发布和数据变更流程增加生产只读基线、依赖全集、隔离演练、停止条件及精确批准范围的前置检查。

### 修复

- 修复叶子等待审批期间重复提交报告、报告处理后通知失败导致重复执行，以及并发唤醒遗漏后续报告的问题。
- 修复协调审阅在重启、重试耗尽和通知恢复时重复调用模型或产生孤立请求的问题。
- 修复活动心跳导致状态短答快照无意义失效，以及工具 Schema 与运行时 Zod 契约漂移的问题。

## [0.5.13] - 2026-09-08

### 新增

- 常驻会话将每轮群消息归入持久 Topic，并按 Topic 独立推进、恢复和关联叶子 Task；任务只保留 Topic 引用，话题页同步展示关联任务与最后活动时间。
- 叶子任务支持从插件配置中按目标组合加载多个任务流程提示词，并在上下文压缩后按已记录的任务索引重新注入；内置通用处理规范与用户补充内容分离，任务流程配置默认折叠展示。

### 变更

- 收敛常驻主会话、Topic 与叶子 Task 的职责边界：主会话负责沟通协调，Topic 负责消息归属和进度，叶子会话按任务目标执行且开放完整工具权限。
...
- 收紧群聊消息的话题归属判定；当消息只是向他人询问分支、草稿或自行调整等协作事项时，不再误归到当前 Task，同时保留引用链和明确任务指令的关联能力。
- Topic 路由、回复审阅和检查点工具改为先构造参数对象再序列化，并对 `undefined` 等非无损值做结构化投影；版本过期、未知请求和无损 JSON 校验失败时返回可恢复的纠错上下文。
- 修复 Resident 任务状态、按需上下文读取、检查点纠错和 UAT 重构建判定，避免错误完成、跨 Topic 污染、旧构建误判通过以及恢复流程重复消费消息。
...
- Resident 启动恢复失败时不再自动创建替代 Session 或改写群绑定；原 Session 保持不变，Runtime 通过恢复问题进入降级状态，等待显式修复后重启。
- 历史回复审阅候选不再完整注入每条群消息和 Task 通知信封，改由 Resident 仅在准备回复时按请求 ID 读取 Runtime 绑定的 8 条/16 KB 快照；本群 Task 系统索引也改为摘要，完整目标、消息时间线和上下文按 Task ID 读取，避免重复历史持续挤占上下文。Resident 恢复时仍会清理依赖旧内存请求表的待处理协议信封，旧版退出产生的可证明无副作用失败自动迁移到顺序恢复。
- 判断在业务副作用前因重启、无效工具参数、Schema 或暂时上下文故障中断时，改为持久化指数退避自动恢复；同群后续消息在前一条恢复完成前只入 Inbox、不越序 Steer。已开始 Task/Outbox 提交后的异常单独展示为“处理提交失败”，禁止自动重放。
- Agent 活动异常结束但原生 Inbox 仍有群判断请求时，先按群执行一次恢复 Turn；短引用回复按精确引用 ID 识别钉钉补充的前导 @，避免自身代回被再次送入判断。
