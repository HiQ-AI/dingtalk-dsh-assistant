...
UAT 部署、生产发布、数据变更与 UAT 同提交重建采用显式目标白名单和受信平台端口。UAT 部署只确认精确提交在目标环境运行；业务回归、提测与验收另行记录。生产发布在创建 Tag 前等待真人审批，并逐段核对提交到运行镜像的来源链；数据变更在精确 UAT 数据库演练，Bytebase 工单创建并回读后等待该平台真人审批。配置、平台只读检查与执行后回读的要求见[本地受信平台接入说明](docs/ops/trusted-platform-workflows.md)；缺少任何必要能力时，流程目录保持不可发起。
