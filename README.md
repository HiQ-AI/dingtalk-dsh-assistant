...
8. 新消息完成话题关联后，先等待本群已收消息完成归类；同话题消息进入同一个批量意图判断，不同话题各自判断。判断中到达的同话题新消息会使旧候选失效并集合重判。已执行中的任务收到补充时，已应用动作保留，剩余动作重新判断；来源编辑使旧话题约束失效。原任务发送者明确说“暂停/取消/恢复这个任务”且目标 Task 已绑定时，可经过 Host 与持久账双重授权后越过无关消息的归类等待。一个业务 Task 固定一个可恢复的负责会话，负责接收任务事件、按验收项决定推进或等待，并组合排查、方案、开发、UAT 等阶段；每个受信阶段仍由独立 Run 执行。任务级暂停、取消与恢复阻止旧确认或后续 Run 越过控制状态；控制状态落盘后，同库节点领取和外部效果派发立即被阻断。 已完成任务仅更改报告语言时，`report` 意图将偏好记录为原 Task 事件，由 Owner 基于已核验产物重新生成报告，不重跑流程；原有正确报告保留历史，需要纠正时按精确通知身份操作。未固化事项由同一 Task Owner 规划和报告：新任务先由 `task-general-intake` 单代码节点冻结目标和范围，Owner 每次选择一项受信能力，Host 以 `task-general-capability` 单代码节点 Run 执行并独立回读，再由 Owner 判断下一步或完成；Host 还会独立核验整体目标。旧 `task-general` 多节点定义仅用于历史 Run 恢复。默认可读取话题原文和本任务前序产物、把原文逐字整理为 Markdown；仅这一类纯整理目标有默认完成核验。数据库排查、文件写入及其他目标缺少已注册能力或目标验收器时阻塞，不宣称完成。迁移及本地验证见[执行底座运维说明](docs/ops/execution-foundation-local.md)。
