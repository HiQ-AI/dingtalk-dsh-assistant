...
- 任务看板：按待执行、执行中、等待中、已完成展示 Task，并打开 DSH 原生叶子对话和轨迹。活动任务卡片中的“任务”面板默认收起，只显示完成数/总数和进度；展开后显示各阶段任务、状态和耗时。每张任务卡还可按需展开 “Self-improving 观测”，读取该 Task 的触发、主题深读、采用、现场验证和效果记录；观测只含脱敏元数据和稳定引用。升级前的 Task 没有结构化记录时明确显示“旧数据未观测”，不把缺失数据判为未触发或无效果。该接口不进入看板的 5 秒全局轮询。执行轮次耗时统计不占用任务卡片空间，仍可通过 `/state/task-timings` 接口用于诊断。
