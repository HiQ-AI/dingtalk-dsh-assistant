...
UAT 部署、生产发布、数据变更与 UAT 同提交重建采用显式目标白名单和受信平台端口。UAT 部署只确认精确提交在目标环境运行；业务回归、提测与验收另行记录。生产发布在创建 Tag 前等待真人审批，并逐段核对提交到运行镜像的来源链；数据变更经本机 PostgreSQL 连接对精确 UAT 库审查 SQL、事务演练并回滚，生产结构从天翼云只读副本回读；Bytebase 先创建 Sheet/Plan/Issue，在 Assistant 任务页真人审批后才创建 Rollout/Task 并执行。配置、平台只读检查与执行后回读的要求见[本地受信平台接入说明](docs/ops/trusted-platform-workflows.md)；缺少任何必要能力时，流程目录保持不可发起。
...
- 任务看板：按待执行、执行中、等待中、已完成展示 Task，并打开 DSH 原生叶子对话和轨迹。无任务负责人会话且计划已成功的任务显示已完成，结果中保留“无法确认”等信息边界，不额外制造等待；有负责人会话的任务仍按当前版本验收结果展示。活动任务卡片中的“任务”面板默认收起，只显示完成数/总数和进度；展开后显示各阶段任务、状态和耗时。完成结果保留在卡片，完成通知已送达时不重复显示，待送达或受阻时显示状态；协调请求重试耗尽显示“系统协调受阻 · 待恢复”。执行轮次耗时统计不占用任务卡片空间，仍可通过 `/state/task-timings` 接口用于诊断。
...
当前执行控制库使用 schema v5；旧 v4 库须按[话题事实离线迁移](docs/ops/execution-foundation-local.md#话题事实-v4--v5-离线迁移)停机、自检、备份、转换并独立回读，服务启动不自动升级。话题事实按来源与状态保存于 `message_topic_facts`，历史不再受整个话题最多 256 条限制；`message.topic.facts` 提供分页原事实，`message.topic` 的前 256 条有效事实视图以 `hasMoreFacts` 标记未返回部分。来源失效与事实变化递增 `contextRevision`，接纳时拒绝基于过期上下文的判断。存储可增长不等于单次模型输入无限，必要上下文仍超限时保持明确阻塞并保留依据。

### 话题上下文与处理过程

消息拆分、关联和话题意图批次仍使用独立判断。IB 的公共话题上下文仅提供一次，并保存来源清单、上下文摘要和任务事实版本；接纳事务再次核对任务状态、控制和授权版本。同一发送人逐字相同的事实只在当前投影合并，完整历史仍保留；不同发送人或不同条件不会合并。原提出人明确撤销条件时，记录旧事实的替代来源和范围，其他发送人不能借话题关联撤销限制。

旧话题和任务候选按游标检索，R 每页核对最多 8 个普通候选，明确引用优先保护。超过 12000 字节的必要材料先分块提取逐字引文并记录覆盖范围；未完成覆盖、伪造引文、歧义或剩余条件仍超限时不会派发。所有分页与材料节点共同受已配置的累计调用和 token 预算约束；默认最多 21 次调用，不为每页重置预算。模型提取仍可能语义遗漏，覆盖记录不能证明理解完全正确。

看板“群消息”的“处理过程”直接展示步骤、判断结论、耗时与同批来源消息，不传输或渲染原始技术详情。话题页可独立翻阅事实与判断批次。任务详情以编号时间线展示执行进展，突出等待事项与最新产出；历史执行和已绑定会话按需展开加载，未绑定会话不提供跳转。最新产出与消息送达状态分开展示。只读接口及游标合同见[工作流接口说明](docs/api/workflow-node-contracts.md#workflow-v2-上下文只读接口)。
