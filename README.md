...
6. 运行看板中的新任务点击后查看节点执行详情：每个节点的状态、等待原因、产出记录和任务最终结果。节点确有模型会话时可从该节点打开会话记录；旧任务继续使用原有会话入口，卡片上的任务进度样式不变。工程发现流程把路径按目录编组传给文件选择节点，保留全部可选路径及 32 KiB 上限；清单确实超限时仍停在可见的容量等待，不裁剪路径。
