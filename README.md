...
- 任务看板：按待执行、执行中、等待中、已完成展示 Task，并打开 DSH 原生叶子对话和轨迹。活动任务卡片中的“任务”面板默认收起，只显示完成数/总数和进度；展开后显示各阶段任务、状态和耗时。卡片另按当前执行轮次展示总墙钟、排队、运行、等待、可配对工具时间和其他运行时间；旧 Task 缺少完整状态或工具事件时明确标为估算，不把生命周期总时长误写成执行时长。
