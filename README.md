...
新消息先逐条立即持久化到 Inbox；同群消息在 5 秒静默窗口内组成一个信息批次，窗口内每收到一条消息就重新计时。批次结束后，即使 resident 正在运行，也只通过一次 DSH `steer` 在下一个 step 边界注入整批消息，并只请求一次结构化判断、最多发送一次群回复。Task 创建、上下文追加和重开按群串行收口；批次内全部原文共同作为 Task 来源证据，最后一条消息作为主要回复引用。失败消息重试会重新加入当时的信息批次，不按单条消息注入特殊重试指令或逐条重判。
