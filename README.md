...
- 任务看板：按待执行、执行中、等待中、已完成展示 Task，并打开 DSH 原生叶子对话和轨迹。活动任务卡片中的“任务”面板默认收起，只显示完成数/总数和进度；展开后显示各阶段任务、状态和耗时。任务结果与完成通知并列展示；协调请求重试耗尽时先显示可读原因，技术错误码可展开查看。执行轮次耗时统计不占用任务卡片空间，仍可通过 `/state/task-timings` 接口用于诊断。
...
- 归档任务：查看已归档 Task，相关群消息仍可重新打开原任务。任务卡片默认展示清理状态、工作目录数和文档数；展开后查看已登记 worktree、文档归档位置及清理失败原因，并可重试归档。
