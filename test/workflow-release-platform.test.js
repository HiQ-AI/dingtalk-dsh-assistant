...
const target = { kind: 'uat-deployment', repository: 'HiQ-AI/dataset', environment: 'uat', service: 'dataset',
...
const requirement = { request: '部署 UAT2', target: { repository: target.repository, environment: 'uat', service: 'dataset',
...
function fixture(overrides = {}) {
  ...
  const clients = {
    ...
    attestations: { async read() { throw new Error('部署不得要求业务 E2E attestation') } },
  }
  return { platform: createReleasePlatform({ targets: [target], clients }), clients, state }
}

test('新增独立 UAT 部署目标不改变已注册生产与重建适配器的规则身份', () => {
  const prod = { ...target, kind: 'production-release', environment: 'production', branch: 'main',
    productionTriggerVerified: true }
  const rebuild = { ...target, kind: 'uat-rebuild' }
  const clients = fixture().clients
  const before = createReleasePlatform({ targets: [prod, rebuild], clients })
  const after = createReleasePlatform({ targets: [prod, rebuild, target], clients })
  for (const kind of ['production-release', 'uat-rebuild']) {
    assert.equal(after.releaseAdapters[kind].rulesDigest, before.releaseAdapters[kind].rulesDigest)
  }
  assert.notEqual(after.releaseAdapters['uat-deployment'].rulesDigest, before.releaseAdapters['uat-rebuild'].rulesDigest)
})
...
test('目标固定白名单，生产触发未证实则拒绝注册', () => {
  ...
  assert.deepEqual(platform.configuredKinds, ['uat-deployment'])
  assert.throws(() => platform.targetFor({ ...requirement, target: { ...requirement.target,
    repository: 'Other/repo' } }, 'uat-deployment'), { code: 'RELEASE_PLATFORM_TARGET_NOT_ALLOWED' })
...
test('没有唯一且已合入的精确 PR 身份，UAT 部署拒绝准备', async () => {
  ...
  await assert.rejects(platform.releaseAdapters['uat-deployment'].inspect({ phase: 'preflight', requirement }),
    { code: 'RELEASE_PLATFORM_PR_IDENTITY_UNCONFIRMED' })
})

test('部署预检只要求目标分支、已合入 PR 与完整流水线扫描', async () => {
  const good = fixture()
  const observed = await good.platform.releaseAdapters['uat-deployment'].inspect({ phase: 'preflight', requirement })
  assert.equal(observed.facts.targetBranchVerified, true)
  assert.equal(observed.facts.uatPrMerged, true)
  assert.equal(observed.facts.localE2ePassed, undefined)
  await assert.rejects(fixture({ branchSha: 'c'.repeat(40) }).platform.releaseAdapters['uat-deployment']
    .inspect({ phase: 'preflight', requirement }), { code: 'RELEASE_PLATFORM_BRANCH_MOVED' })
  await assert.rejects(fixture({ complete: false }).platform.releaseAdapters['uat-deployment']
...
test('精确 prepared 才能触发，写回执之后必须独立回读成功', async () => {
  ...
  const adapter = platform.releaseAdapters['uat-deployment']
  const observation = { phase: 'preflight', status: 'confirmed', targetDigest: executionDigest(requirement.target), evidenceRefs: ['github:branch:1'] }
  const expected = { commitSha, previousPhase: 'preflight',
    previousEvidenceDigest: executionDigest(observation.evidenceRefs) }
  const prepared = await adapter.prepareOperation({ kind: 'uat-deployment', operation: 'build', requirement,
...
test('流水线未完成时回读 unknown，不发送第二次', async () => {
  ...
  const prepared = await adapter.prepareOperation({ kind: 'uat-deployment', operation: 'build', requirement,
    ...
    expected: { commitSha, previousPhase: 'preflight', previousEvidenceDigest: executionDigest(observation.evidenceRefs) } })
...
test('执行前再查：已有在途流水线不重触发，普通 UAT 失败须走重建', async () => {
  const built = fixture()
  built.state.pipeline.push({ number: 4, branch: target.branch, commitSha, status: 'success' })
  const builtPreflight = await built.platform.releaseAdapters['uat-deployment'].inspect({ phase: 'preflight', requirement })
  assert.equal(builtPreflight.facts.uatPrMerged, true)
  const builtPrepared = await built.platform.releaseAdapters['uat-deployment'].prepareOperation({ kind: 'uat-deployment',
    operation: 'build', requirement, observation: builtPreflight, runId: 'run-existing', generation: 1,
    requirementDigest: executionDigest(requirement), expected: { commitSha, previousPhase: 'preflight',
      previousEvidenceDigest: executionDigest(builtPreflight.evidenceRefs) } })
  assert.equal((await built.platform.operationAdapter.execute(builtPrepared)).status, 'succeeded')
  assert.equal(built.state.calls.length, 0)
  const ongoing = fixture()
  ongoing.state.pipeline.push({ number: 5, branch: target.branch, commitSha, status: 'running' })
  const observation = { phase: 'preflight', status: 'confirmed',
    targetDigest: executionDigest(requirement.target), evidenceRefs: ['github:branch:1'] }
  const prepared = await ongoing.platform.releaseAdapters['uat-deployment'].prepareOperation({ kind: 'uat-deployment',
  ...
  await assert.rejects(failed.platform.releaseAdapters['uat-deployment'].inspect({ phase: 'preflight', requirement }),
...
test('运行态凭流水线 SHA→镜像摘要→Pod imageID 同链，Deployment 无源码注解也可确认', async () => {
  ...
  const effect = { prepared: { workflowKind: 'uat-deployment', targetDigest: executionDigest(requirement.target),
    operationKey: 'operation-1' }, receipt: { status: 'succeeded' } }
  const observed = await good.platform.releaseAdapters['uat-deployment'].inspect({ phase: 'runtime', requirement, effect })
  ...
  await assert.rejects(drift.platform.releaseAdapters['uat-deployment'].inspect({ phase: 'runtime', requirement, effect }),
  ...
  await assert.rejects(missing.platform.releaseAdapters['uat-deployment'].inspect({ phase: 'runtime', requirement, effect }),
