...
test('真实SDK JSON迁移check零写、独立目标读回、重复运行与旧版本回退可读', async (t) => {
  ...
  assert.equal(JSON.parse(await readFile(target, 'utf8')).unit.version, residentDomainSpec.version)
  ...
  await assert.rejects(oldBackend.kv.open({ name: residentDomainSpec.name, version: residentDomainSpec.version, tables: Object.keys(residentDomainSpec.tables), hasGlobal: false }), { code: 'version-mismatch' })
