...
test('归类可新建、追加或多归属，空归属必须有原因', () => {
  ...
  assert.deepEqual(topicRouteSubmissionSchema.parse({ requestId: 'route-a', routes: [route] }).routes, [{ ...route, ignoredRefs: [] }])
...
test('显式任务指向识别配置名称、别名、DWS登录人或cc指令', () => {
  ...
  assert.equal(isExplicitAgentDirection('@当前登录人(当前登录人) 需要修复', ['数字助理']), false)
  assert.equal(isDirectedToOtherParticipants('@当前登录人(当前登录人) 需要修复', names), false)
  assert.equal(isDirectedToOtherParticipants('@其他同事 需要修复', names), true)
...
test('主会话对任务补充发送简短且不复述信息的确认', async () => {
  ...
  assert.doesNotMatch(source, /仅因消息提及当前 DWS 登录人姓名/u)
