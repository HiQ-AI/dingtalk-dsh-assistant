...
function workflow(overrides = {}, completionCheck = async ({ evidence, report }) => evidence[0]?.output.value === report.summary) {
  const calls = []
  const definition = createGeneralTaskWorkflow({ provider: 'test', model: 'synthetic', maxSteps: 2,
    ...
    capabilities: [{ id: 'lookup', description: '查询指定范围的资料', identity: 'lookup-v1',
      ...
      verify: async args => { calls.push(['verify', args]); return { passed: true, readback: '实际读取值' } },
      ...overrides }],
  })
...
test('通用流程只调用登记能力，逐步独立核验并保存证据', async () => {
  ...
  const requirement = { request: '查询一份资料', constraints: [], scope: { folder: 'docs' } }
...
test('核验失败不能形成任务证据，预算终点不得无依据宣称完成', async () => {
  ...
  const state = await definition.nodes[0].execute({ input: { request: '查询资料', constraints: [], scope: {} } })
  const execute = definition.nodes.find(node => node.id === 'execute-1')
...
test('模型标记完成且附带证据时，Host 验收失败仍拒绝完成', async () => {
  const { definition } = workflow({}, async () => false)
  const initial = await definition.nodes[0].execute({ input: { request: '查询资料', constraints: [], scope: {} } })
...
test('真实控制账将只读步骤、回读和报告作为独立检查点', async t => {
  ...
  await controller.createRun({ commandId: 'create', runId: 'run', taskId: 'task', workflowId: 'task-general', input: { request: '查询资料', constraints: [], scope: { folder: 'docs' } } })
...
test('缺能力或预算耗尽停在 waiting，并保留受阻报告', async t => {
  ...
  const controller = createExecutionController({ store, artifacts, sessions, workflows: [definition] })
  t.after(async () => { await controller.close(); await store.close(); await rm(root, { recursive: true, force: true }) })
  await controller.createRun({ commandId: 'create', runId: 'run', taskId: 'task', workflowId: 'task-general', input: { request: '查询资料', constraints: [], scope: {} } })
  const state = await controller.whenIdle('run')
