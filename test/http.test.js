...
async function withServer(testApiEnabled, run, { transport = 'fake-dws', getDwsBridgeHealth, overrides = {} } = {}) {
  const runtime = {
    listRecoveryIssues: () => [], listGroups: () => [], getGroup: () => undefined, listTasks: () => [], listTaskTimings: () => [{ taskId: 'task-1', wallMs: 1000 }], listActivities: () => [], listSelfImprovingObservations: (taskId) => [{ observationId: 'observation-1', taskId }], listAlerts: () => [], listAuthorizationRequests: () => [{ requestId: 'blocker-1', status: 'pending-send' }],
...
test('生产HTTP开放只读状态与明确的本机群配置接口，测试控制面仍关闭', async () => withServer(false, async (baseUrl) => {
  ...
  assert.deepEqual(await (await fetch(`${baseUrl}/state/self-improving-observations?taskId=task-1`)).json(), [{ observationId: 'observation-1', taskId: 'task-1' }])
