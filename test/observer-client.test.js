...
test('运行看板保留左侧菜单并替换右侧整体内容', async () => {
  ...
  assert.match(source, /padding: `0 0 \$\{paddingBottom\}px`[\s\S]*fontSize: 13, fontWeight: 500, lineHeight: '16px'/)
  ...
  assert.match(source, /repeat\(4, minmax\(300px, 1fr\)\)', gap: 10/)
  ...
  assert.match(source, /const latestRemainingItems = latestCheckpoint\?\.remainingItems \|\| checkpoints/)
  assert.match(source, /const remainingItemsMatchPlan = latestRemainingItems\.every\(\(checkpoint\) => checkpoints\.includes\(checkpoint\)\)/)
  assert.match(source, /const normalizedRemainingItems = remainingItemsMatchPlan \? latestRemainingItems : checkpoints\.slice\(-Math\.min\(checkpoints\.length, latestRemainingItems\.length\)\)/)
  assert.match(source, /const remainingCheckpointNames = new Set\(normalizedRemainingItems\)/)
  ...
  assert.match(source, /label: '筛选审批状态'[\s\S]*?fitContent: true \}\)\),/)
  ...
  assert.match(source, /borderBottom: selected \? '2px solid #4d6bfe'[\s\S]*color: selected \? '#4d6bfe'/)
