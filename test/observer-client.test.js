...
test('运行看板保留左侧菜单并替换右侧整体内容', async () => {
  ...
  assert.match(source, /const completedCheckpointNames = new Set\(task\.state === 'completed'/)
  assert.match(source, /item\.kind === 'stage-completed' && \['acknowledge', 'guidance'\]\.includes\(item\.coordinatorDecision\)/)
  ...
  assert.match(source, /const checkpoints = task\.stageTasks\?\.length \? task\.stageTasks/)
