...
test('运行看板保留左侧菜单并替换右侧整体内容', async () => {
  ...
  assert.match(source, /onClick: refresh, style: \{ gap: 6 \} \}, '刷新', refreshIcon/)
  ...
  assert.match(source, /position: 'fixed', inset: 0, paddingLeft: sidebarWidth, boxSizing: 'border-box', zIndex: 900/)
  assert.match(source, /overlayRef\.current\.style\.paddingLeft/)
  ...
  assert.match(source, /const authorizationTag =/)
  assert.match(source, /minWidth: 58, minHeight: 22/)
  assert.match(source, /borderRadius: 6, background:/)
  assert.match(source, /React\.createElement\(StateDot, \{ state, size: 6 \}\)/)
  ...
  assert.match(source, /useLayoutEffect, useRef/)
  assert.match(source, /const right = sidebar\?\.getBoundingClientRect\(\)\.right/)
  assert.match(source, /overlayRef\.current\.style\.paddingLeft = `\$\{roundedRight\}px`/)
