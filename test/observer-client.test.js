...
test('运行看板保留左侧菜单并替换右侧整体内容', async () => {
  ...
  assert.match(source, /fontSize: 14, fontWeight: 500, lineHeight: '20px'[\s\S]*'运行看板'/)
  assert.match(source, /height: 27[\s\S]*gap: 36[\s\S]*padding: '0 0 0 8px'/)
  assert.match(source, /fontSize: 13, fontWeight: 500, lineHeight: '16px'/)
  ...
  assert.match(source, /border: `1px solid \$\{colors\.border\}`, borderRadius: 10, background: `color-mix\(in srgb, \$\{colors\.muted\} 6%, \$\{colors\.cardSurface\}\)`, overflow: 'hidden'/)
