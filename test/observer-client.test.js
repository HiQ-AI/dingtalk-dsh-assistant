...
test('运行看板保留左侧菜单并替换右侧整体内容', async () => {
  ...
  assert.match(source, /height: 44, boxSizing: 'border-box'/)
  ...
  assert.match(source, /height: 31, display: 'flex'/)
  assert.match(source, /const pageHeader = \(title, description\)/)
  assert.doesNotMatch(source, /Pill, null, String\(count\)/)
  assert.match(source, /const main = React\.createElement\('main', \{ style: \{ width: '100%'/)
  assert.match(source, /maxWidth: activePage === 'tasks' \? 'none' : 1320/)
  ...
  assert.match(source, /justifySelf: 'center', width: 28, height: 18/)
  assert.match(source, /data-task-card-action.*toggle-checkpoints/)
  assert.match(source, /maxWidth: activePage === 'tasks' \? 'none' : 1320.*margin: '0 auto'/)
  ...
  assert.match(source, /remainingCheckpointNames = new Set\(latestCheckpoint\?\.remainingItems \|\| checkpoints\)/)
  ...
  assert.match(source, /checkpointDuration\(checkpoint, currentCheckpointEvents, Date\.now\(\)\)/)
  ...
  assert.match(source, /const checkpoints = planCheckpoint\?\.remainingItems\?\.length \? planCheckpoint\.remainingItems/)
  ...
  assert.match(source, /justifySelf: 'start'[\s\S]*statusTag\(status\.label, status\.state\)/)
  assert.match(source, /gridTemplateColumns: 'max-content minmax\(260px, 1\.1fr\)/)
  assert.match(source, /copyButton\(task\.taskId, ' Id'\)/)
  ...
  assert.match(source, /WebkitLineClamp: 3/)
