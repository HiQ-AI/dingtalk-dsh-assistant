...
const wasInserted = (session, id) => session.events.some((event) => event.type === 'agent/inbox/spliced' && event.data.inserted.some((message) => message.id === id))
const enteredSurface = (session, id) => session.events.some((event) => event.type === 'user/message' && event.data.id === id)
...
async function reopen(t, session) {
  ...
  try { await handle.writeFile(JSON.stringify({ header: session.header, events: session.events })); await handle.sync() } finally { await handle.close() }
...
test('原生Inbox已进入user/message后跨重开无pending，但稳定ID仍可确认曾下发', async (t) => {
  ...
  const event = restored.session.events.find((item) => item.type === 'user/message')
...
test('原生Inbox取消保留历史inserted，不能凭此认定输入已消费或仍待执行', async (t) => {
  ...
  assert.equal(wasInserted(restored.session, message.id), true)
  assert.equal(enteredSurface(restored.session, message.id), false)
  const cancellation = restored.session.events.find((event) => event.type === 'agent/inbox/spliced' && event.data.outcome === 'canceled')
...
test('原生Inbox仅claim未进入surface时重开可见差异，不声称叶子已阅读', async (t) => {
  ...
  assert.equal(wasInserted(restored.session, message.id), true)
  assert.equal(enteredSurface(restored.session, message.id), false)
  assert.equal(restored.session.events.some((event) => event.type === 'agent/inbox/spliced' && event.data.outcome === 'canceled'), false)
})
